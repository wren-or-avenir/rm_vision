...
class Detector:
    ...
    def preprocessing(self, img, val):
    ...
    # 轮廓检测并绘制外轮廓
    def detect_contours(self, img, color_img):
    ...
    # 绘制灯带最小外接矩形，用面积做初筛
    def draw_min_rect(self, img, color_img, target_color1_range=None, target_color2_range=None):
    ...
    def filter_by_ratio_and_angle_with_adjust(self, color_img, rect_info_list):
        ...
        for rect_info in rect_info_list:
            ...
            adjusted_w, adjusted_h, adjusted_angle = self.adjust(original_rect)
    ...
    # 接下来配对每组灯带，进行匹配    
    def is_close(self, rect1, rect2):
        (x1, y1), (w1, h1), angle1 = rect1
        (x2, y2), (w2, h2), angle2 = rect2
        # 使用adjust函数调整矩形参数
        adjusted_w1, adjusted_h1, adjusted_angle1 = self.adjust(((x1, y1), (w1, h1), angle1))
        adjusted_w2, adjusted_h2, adjusted_angle2 = self.adjust(((x2, y2), (w2, h2), angle2))
    ...
    def draw_paired_lights(self, color_img, filtered_rects):
        """
        根据配对条件筛选并绘制配对的灯条
        """
        result_img = color_img.copy()
        ...
        for i in range(len(rect_list)):
            ...
            for j in range(i + 1, len(rect_list)):
                ...
                if self.is_close(rect_list[i], rect_list[j]):
...
if __name__ == "__main__":
    ...
    #显示原图
    detector.show_image(original_img)                            

    #轮廓检测前的预处理
    preprocessed_img = detector.preprocessing(original_img, 140)
    detector.show_image(preprocessed_img) 

    #轮廓检测并绘制外轮廓
    detected_contours = detector.detect_contours(preprocessed_img, original_img)
    detector.show_image(detected_contours)
    #绘制最小外接矩形并做面积初筛
    drawn_min_rect_img, rect_info_list = detector.draw_min_rect(preprocessed_img, original_img)
    detector.show_image(drawn_min_rect_img)
    ...
    # 根据长宽比和角度筛选矩形，并绘制筛选结果
    filtered_img, filtered_rects = detector.filter_by_ratio_and_angle_with_adjust(original_img, rect_info_list)
    detector.show_image(filtered_img)
    ...
    # 绘制配对的灯条
    paired_img, paired_groups = detector.draw_paired_lights(original_img, filtered_rects)
    detector.show_image(paired_img)
